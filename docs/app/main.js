import { fields, roles, dateFields, addressFields, displayName, buildRecord, upgradeRecord, recordValues, combinedDocument, MODEL_VERSION, NS, JSONLD_CONTEXT } from './model.js';
...
const primary=['Name','First Name','Middle Name','Last Name','Nickname','Birthday','Phone','E-mail','Address',...addressFields.map(([field])=>field),'Church name','Membership role','Baptism date','Membership date','Transferred date'];
const fieldLabels=Object.fromEntries(addressFields.map(([field,,label])=>[field,label]));
for(const name of [...primary,...fields.filter(k=>!primary.includes(k))]) {
  const label=document.createElement('label');label.textContent=fieldLabels[name] || (name==='Name'?'Full / display name':name==='Address'?'Original / unstructured address':name);
  ...
  else if(input.tagName==='INPUT') { input.type=dateFields.includes(name)?'date':'text'; if(name==='Phone')input.inputMode='tel'; if(name==='E-mail')input.inputMode='email'; }
...
function resetPersonForm(){HTMLFormElement.prototype.reset.call($('person-form'));}
function edit(record) {editing=record?.id||null;const v=record?recordValues(record):{};resetPersonForm();for(const input of $('person-form').elements)if(input.name)input.value=v[input.name]||'';dirty=false;$('form-title').textContent=record?'Edit person':'Add a person';$('record-id').textContent=editing||'';$('delete').hidden=!record;renderList();}
$('person-form').oninput=()=>{dirty=true;};
$('person-form').onsubmit=run(async e=>{e.preventDefault();const v=Object.fromEntries(new FormData(e.target));const old=records.find(r=>r.id===editing);const record=buildRecord(v,old,{preserveSource:false});await parseRdfTextWithAdapters(JSON.stringify(record.document),{format:'jsonld'});if(old)records=records.map(r=>r.id===old.id?record:r);else records.push(record);edit(record);if(persist())notice(`${displayName(v)} saved. Download the graph for a portable copy.`);});
...
try {const saved=localStorage.getItem(key);if(saved){const parsed=JSON.parse(saved);if(!Array.isArray(parsed)||parsed.some(r=>!r.document?.['@graph']||!r.id||!r.ids))throw Error('Invalid saved records');records=parsed.map(upgradeRecord);$('persist').checked=true;if(!parsed.some(r=>r.modelVersion!==MODEL_VERSION)||persist())notice(`Restored ${records.length} records from this device.`);}}catch(e){notice('The saved device copy could not be read. It has not been overwritten.',true);}
...
roles.forEach(([value,label])=>$('import-role').append(option(value,label)));
$('import-church').oninput=()=>invalidate();$('import-role').onchange=()=>invalidate();
...
$('validate-import').onclick=run(()=>{invalidate();const mapping=[...$('mapping').querySelectorAll('select')].map(s=>s.value);prepared=prepareImport(rows,$('header-row').checked,mapping,{'Church name':$('import-church').value,'Membership role':$('import-role').value});if(prepared.errors.length){$('import-errors').textContent=prepared.errors.join('\n');notice('Import has errors. Correct the source file or mapping and validate again.',true);}else if(!prepared.records.length){notice('No records to import.',true);}else{$('commit-import').disabled=false;notice(`${prepared.records.length} people validated. Ready to add.`);}});
...
async function serialize(){const parsed=await parseRdfTextWithAdapters(JSON.stringify(combinedDocument(records)),{format:'jsonld'});const result=await serializeRdfDatasetWithAdapters(parsed.dataset,{format:$('format').value,context:JSONLD_CONTEXT,prefixes:NS});return {...result,count:parsed.quads.length};}
